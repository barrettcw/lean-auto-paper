...
\begin{definition} Suppose the term $a_0 \ a_1 \ \dots \ a_k$ is type correct under
  ...
  $$\mathsf{DArgs}(\Gamma; a_0, (a_1, \dots, a_k)) := (b_{i_1}, b_{i_2}, \dots, b_{i_m})$$
...
\begin{definition} We define the predicate $\mathsf{QMono}(\Gamma; V, t)$ inductively,
  ...
  \begin{enumerate}
    \item For variable $x \in V$ and terms $t_1, t_2, \dots, t_n$,
      \begin{align*}
      ...
      \end{align*}
    \item For variable $x \notin V$ and terms $t_1, t_2, \dots, t_n$,
      \begin{align*}
...
whether $t$ is quasi-monomorphic and fails if it's not, but for simplicity, these checks have been omitted in $\mathsf{lamAbst}$.
  ...
  \Fn{\lamAbstraction{$\Gamma; V, t$}}{
    ...
    \Switch(\textbf{with}){t}{
      \Case(\tcc*[h]{Function application}){$a \ b$}{
        ...
        \uIf{$f \in V$}{
          \For{a : args}{$a := \mathsf{lamAbst}(a)$}
          \Return $\mathsf{mkAppN}(f, args)$
        }
        $lf := \mathsf{LFun}(\Gamma; f, args)$ \;
        $largs := \mathsf{LArgs}(\Gamma; f, args)$ \;
        $lvar := \mathsf{getLVarName}(t)$ \;
        \Return $\mathsf{mkAppN}(lvar, largs)$
      }
      \Case{$\forall (v : a). b$}{
        $atype := \mathsf{inferType}(\Gamma; a)$ \;
        $babst := \mathsf{lamAbst}(\Gamma, v : a; V \cup \{v\}, b)$ \;
        \uIf{$atype = \mathsf{U}_0$}{
          $aabst := \mathsf{lamAbst}(\Gamma; V, a)$ \;
          \Return $aabst \to babst$
        }
