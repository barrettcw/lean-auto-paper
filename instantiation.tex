...
\begin{algorithm}\label{saturate}
  ...
  \Fn{\saturateFun{$\Gamma; H, maxInsts$}}{
    \Input{$\lambda C$ context $\Gamma$, list of $\lambda C$ terms $H$, and threshold $maxInsts$}
    \Output{A list of $\lambda C$ terms}
    $hi := H$ \tcc*[h]{List of hypothesis instances} \;
    $ci := \mathsf{Queue.empty}()$ \tcc*[h]{A queue of constant instances} \;
    \For{h : H}{
      \For{$m : \mathsf{monoInsts}(\Gamma; \emptyset, h)$}{$ci.\mathsf{push}(m)$}
    }
    $activeci := ci.\mathsf{copy}()$ \;
    \While{$! \ activeci.\mathsf{empty}()$}{
      $m := activeci.\mathsf{front}()$ \;
      $activeci.\mathsf{popFront}()$ \;
      $prevhi = hi.\mathsf{copy}()$ \;
      \For{h : prevhi}{
        $newHi := \mathsf{matchInst}(\Gamma; m, h)$ \;
        \For{nh : newHi}{
          \lIf{$nh \in hi$}{\Continue}
          $hi.\mathsf{push}(nh)$ \;
          $newci := \mathsf{monoInsts}(\Gamma; \emptyset, nh)$ \;
          \For{nm : newci}{
            \lIf{$nm \in ci$}{\Continue}
            $ci.\mathsf{push}(nm)$ \;
            $activeci.\mathsf{push}(nm)$
          }
        }
        \lIf{$hi.\mathsf{size}() + ci.\mathsf{size}() > maxInsts$}{\Break}
      }
      \lIf{$hi.\mathsf{size}() + ci.\mathsf{size}() > maxInsts$}{\Break}
    }
    $monohi := \mathsf{List.empty}()$ \;
    \For{h : hi}{
      \lIf{$\mathsf{QMono}(\Gamma; \emptyset, h)$}{$monohi.\mathsf{push}(h)$}
    }
    \Return $monohi$
  }
\end{algorithm}
